...
class TypeDefinitionManager
{
     ...
     */
    public function getTypeDefinition(int $recordType): TypeDefinition
    {
        return $this->typeDefs[$recordType]
            ?? ($this->typeDefs[$recordType] = new TypeDefinition(self::DEFINITIONS[$recordType] ?? ['data' => Types::ANYTHING]));
    }
    ...
    public function registerTypeDefinition(int $recordType, $definition)
    {
        if (!($definition instanceof TypeDefinition)) {
            if (!\is_array($definition)) {
                throw new \InvalidArgumentException('Definition must be an array or an instance of ' . TypeDefinition::class);
            }

            $definition = new TypeDefinition($definition);
        }
